...
export function SendPoolConfirm(props) {
    ...
    const handleSend = async () => {
        handleToggleDialog();
        const result = await execute({ path: "/admin/survey", requestMethod: "POST", data: {frequency: parseInt(value)} });
        ...
        <>
            <Dialog onClose={handleCloseDialog} open={props.openDialog}>
                ...
                <Box sx={{ margin: 3 }}>
                    ...
                    <Button sx={{mx:2}} color="error" variant="contained" onClick={handleCloseDialog} >
                            Anuluj
                        </Button>
